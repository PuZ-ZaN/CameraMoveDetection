...
def main():
	...
	while fvs.more() != None:
		try:
			...
			difference = cv2.subtract(frame, prev_img)
			...
			if(config.ShowVideo):
				...
				#difference[mask != 255] = [128, 128, 128]
				
				cv2.putText(difference, "Fps: {}".format(elapsedSecs), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
				cv2.imshow(config.NameVideoWindow, difference)
